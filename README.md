...
2 important variables are declared:
turn = 0
This determines which player's turn it is.
players = sizeof(names) / sizeof(names[0])
This determines how many players are in the game, for usage in the following 2 functions.
...
The following actions infinitely loop until the game ends:
  The balance array is checked for all numbers other than 0.
    When a number other than 0 is encountered,
      the respective player is recorded as the winner
      the respective balance combined with the pot is recorded as the winnings
  ...
  Otherwise, the game is still running.
    ...
    For each roll the active player has,
      ...
      If it rolled 1,
        the active player gives $1 to the player on their left.
      If it rolled 2,
        the active player gives $1 to the player on their right.
      If it rolled 3,
        the active player adds $1 to to the pot.
      Otherwise, nothing happens.
    After the active player has exhausted all their rolls, they pass their turn to the player on their left.
